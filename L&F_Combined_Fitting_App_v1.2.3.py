...
def analyze_data():
    ...
    try:
        if model_type == "Langmuir":
            ...
            y_means = [np.mean([S_values[i] for i, c in enumerate(C_values) if c == c_val]) for c_val in
                       unique_c_values]

            ax1.errorbar(unique_c_values, y_means, yerr=S_std, fmt='o', label='Selected Data Points', color='#008000',
            ...
            y_means_cs = [np.mean([C_values[i] / S_values[i] for i, c in enumerate(C_values) if c == c_val]) for c_val
                          in unique_c_values]

            ax2.errorbar(unique_c_values, y_means_cs, yerr=CS_std, fmt='o', label='Selected Data Points',
            ...
            y_means = [np.mean([S_values[i] for i, c in enumerate(C_values) if c == c_val]) for c_val in
                       unique_c_values]
            ax3.errorbar(unique_c_values, y_means, yerr=S_std, fmt='o', label='Selected Data Points', color='#008000',
                         markersize=5, capsize=5, ecolor='black', elinewidth=0.8)
            ...
            if len(C_log_values_selected) > 0 and len(S_log_values_selected) > 0:
                ...
                y_means_log = [
                    np.mean([np.log10(S_values[i]) for i, c in enumerate(C_values) if c == c_val and S_values[i] > 0])
                    for c_val in unique_c_values if
                    np.mean([S_values[i] for i, c in enumerate(C_values) if c == c_val]) > 0]

                if len(C_log_values_selected_plot) > 0 and len(S_log_values_selected_plot) > 0:
                    ax4.errorbar(np.log10(unique_c_values), y_means_log, yerr=S_log_std,
