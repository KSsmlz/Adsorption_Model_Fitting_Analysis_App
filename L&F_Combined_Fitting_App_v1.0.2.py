...
def analyze_data():
    ...
    try:
        if model_type == "Langmuir":
            ...
            fig1, ax1 = plt.subplots(figsize=(6, 4))  # Adjust figure size
            ax1.scatter(all_C_values, all_S_values, label='All Data Points', color='blue', alpha=0.5)
            ...
            ax1.set_title('Langmuir Adsorption Isotherm', fontsize=12)
            ax1.legend(fontsize=8)
            ax1.grid(True)


            fig2, ax2 = plt.subplots(figsize=(6, 4))  # Adjust figure size
            ...
            ax2.set_title('Langmuir C/S vs. C Plot', fontsize=12)
            ...
            fig3, ax3 = plt.subplots(figsize=(6, 4))  # Adjust figure size
            ax3.scatter(all_C_values, all_S_values, label='All Data Points', color='blue', alpha = 0.5)
            ...
            ax3.set_title('Freundlich Adsorption Isotherm', fontsize=12)
            ax3.legend(fontsize=8)
            ax3.grid(True)


            fig4, ax4 = plt.subplots(figsize=(6, 4))  # Adjust figure size
            ax4.scatter(np.log10(all_C_values), np.log10(all_S_values), label = 'All Data Points', color = 'blue', alpha = 0.5)
            ...
            ax4.set_title('Log Scale Freundlich Adsorption Isotherm', fontsize=12)
...
def refresh_data_frame():
    ...
    for i, (c, s) in enumerate(data_list):
        tk.Label(data_frame, text=f"{c:.4f}", font = ('Arial', 10)).grid(row=i, column=0, padx=5, pady=2)
        tk.Label(data_frame, text=f"{s:.4f}", font = ('Arial', 10)).grid(row=i, column=1, padx=5, pady=2)
...
tk.Label(window, text="Select Model:", font = ('Arial', 12), background = 'white').grid(row=0, column=0, padx=5, pady=5, sticky='e')
model_var = tk.StringVar(window)
model_var.set("Langmuir")  # Default value
model_dropdown = ttk.Combobox(window, textvariable=model_var, values=["Langmuir", "Freundlich"], font = ('Arial', 12))
model_dropdown.grid(row=0, column=1, padx=5, pady=5, sticky='w')


tk.Label(window, text="Concentration (C):", font = ('Arial', 12), background = 'white').grid(row=1, column=0, padx=5, pady=5, sticky='e')
c_entry = tk.Entry(window, font = ('Arial', 12))
c_entry.grid(row=1, column=1, padx=5, pady=5, sticky='w')

tk.Label(window, text="Adsorption (S):", font = ('Arial', 12), background = 'white').grid(row=2, column=0, padx=5, pady=5, sticky='e')
s_entry = tk.Entry(window, font = ('Arial', 12))
s_entry.grid(row=2, column=1, padx=5, pady=5, sticky='w')

add_button = tk.Button(window, text="Add Data", command=add_data, font = ('Arial', 12))
add_button.grid(row=2, column=2, padx=5, pady=5, sticky='w')

delete_button = tk.Button(window, text="Delete Data", command=delete_data, font = ('Arial', 12))
...
analyze_button = tk.Button(window, text="Analyze Data", command=analyze_data, font = ('Arial', 12))
analyze_button.grid(row=5, column=0, columnspan=3, padx=10, pady=10, sticky='ew')

result_text = tk.Text(window, height=4, width=60, state=tk.DISABLED, font = ('Arial', 12))
