...
def analyze_data():
    ...
    try:
        if model_type == "Langmuir":
            ...
            # Reset view button for Langmuir Isotherm
            def reset_view1():
                ax1.set_xlim(initial_xlim1)
                ax1.set_ylim(initial_ylim1)
                canvas1.draw()

            reset_button1 = ttk.Button(frame1, text="Reset View", command=reset_view1, style='TButton', width=10)
            reset_button1.pack(side=tk.BOTTOM, pady=5)

            # Reset view button for Langmuir C/S vs C plot
            def reset_view2():
                ax2.set_xlim(initial_xlim2)
                ax2.set_ylim(initial_ylim2)
                canvas2.draw()

            reset_button2 = ttk.Button(frame2, text="Reset View", command=reset_view2, style='TButton', width=10)
            reset_button2.pack(side=tk.BOTTOM, pady=5)
            ...
            # Reset view button for Freundlich Isotherm
            def reset_view3():
                ax3.set_xlim(initial_xlim3)
                ax3.set_ylim(initial_ylim3)
                canvas3.draw()
            reset_button3 = ttk.Button(frame3, text="Reset View", command=reset_view3, style='TButton', width=10)
            reset_button3.pack(side=tk.BOTTOM, pady=5)

            # Reset view button for Freundlich log scale plot
            def reset_view4():
                ax4.set_xlim(initial_xlim4)
                ax4.set_ylim(initial_ylim4)
                canvas4.draw()
            reset_button4 = ttk.Button(frame4, text="Reset View", command=reset_view4, style='TButton', width=10)
            reset_button4.pack(side=tk.BOTTOM, pady=5)
