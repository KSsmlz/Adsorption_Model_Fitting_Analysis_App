...
def add_data():
    ...
    if c_values_str and s_values_str:
        try:
            c_values = list(map(float, (c_values_str.replace(',', ' ').split())))
            s_values = list(map(float, (s_values_str.replace(',', ' ').split())))

            if len(c_values) != len(s_values):
                messagebox.showerror("Error", "Number of C and S values do not match.")
                return
...
def refresh_data_frame():
    ...
    tk.Label(data_scrollable_frame, text="C", font=('Arial', 14, 'bold'), bg='#f0f0f0').grid(row=0, column=0, padx=5,
                                                                                             pady=2, sticky='w')
    tk.Label(data_scrollable_frame, text="S", font=('Arial', 14, 'bold'), bg='#f0f0f0').grid(row=0, column=1, padx=5,
                                                                                             pady=2, sticky='w')

    for i, (c, s) in enumerate(data_list):
        tk.Label(data_scrollable_frame, text=f"{c:.4f}", font=('Arial', 14), bg='#f0f0f0').grid(row=i + 1, column=0,
                                                                                                padx=5, pady=2,
                                                                                                sticky='w')
        tk.Label(data_scrollable_frame, text=f"{s:.4f}", font=('Arial', 14), bg='#f0f0f0').grid(row=i + 1, column=1,
                                                                                                padx=5, pady=2,
                                                                                                sticky='w')
        tk.Checkbutton(data_scrollable_frame, variable=check_vars[i], bg='#f0f0f0').grid(row=i + 1, column=2, padx=5,
                                                                                         pady=2, sticky='w')
...
# Function to handle bulk data input
def bulk_c_input():
    bulk_window = tk.Toplevel(window)
    bulk_window.title("Bulk C Input")
    bulk_window.configure(background='white')

    text_area = tk.Text(bulk_window, height=10, width=40, font=('Arial', 12))
    text_area.pack(padx=10, pady=10)

    def process_c_data():
        c_entry.delete(0, 'end')
        c_entry.insert(0, text_area.get("1.0", tk.END).strip())
        bulk_window.destroy()

    submit_button = ttk.Button(bulk_window, text="Submit", command=process_c_data, style='TButton')
    submit_button.pack(pady=10)


def bulk_s_input():
    bulk_window = tk.Toplevel(window)
    bulk_window.title("Bulk S Input")
    bulk_window.configure(background='white')
    ...
    def process_s_data():
        s_entry.delete(0, 'end')
        s_entry.insert(0, text_area.get("1.0", tk.END).strip())
        bulk_window.destroy()

    submit_button = ttk.Button(bulk_window, text="Submit", command=process_s_data, style='TButton')
    submit_button.pack(pady=10)
...
model_dropdown = ttk.Combobox(input_frame, textvariable=model_var, values=["Langmuir", "Freundlich"],
                              font=('Arial', 12), width=18)  # apply font here
...
c_entry = tk.Entry(input_frame, **entry_style, width=18)
c_entry.grid(row=1, column=1, padx=2, pady=2, sticky='w')

bulk_c_button = ttk.Button(input_frame, text="Bulk C", command=bulk_c_input, style='TButton', width=7)
bulk_c_button.grid(row=1, column=2, padx=2, pady=2, sticky='w')

tk.Label(input_frame, text="Adsorption (S):", **label_style).grid(row=2, column=0, padx=2, pady=2, sticky='e')
s_entry = tk.Entry(input_frame, **entry_style, width=18)
s_entry.grid(row=2, column=1, padx=2, pady=2, sticky='w')

bulk_s_button = ttk.Button(input_frame, text="Bulk S", command=bulk_s_input, style='TButton', width=7)
bulk_s_button.grid(row=2, column=2, padx=2, pady=2, sticky='w')

add_button = ttk.Button(input_frame, text="Add", command=add_data, style='TButton', width=7)
add_button.grid(row=3, column=0, padx=2, pady=2, sticky='w')

delete_button = ttk.Button(input_frame, text="Delete", command=delete_data, style='TButton', width=7)
delete_button.grid(row=3, column=1, padx=2, pady=2, sticky='w')

select_all_button = ttk.Button(input_frame, text="Select All", command=toggle_all, style='TButton', width=10)
select_all_button.grid(row=4, column=0, columnspan=2, padx=2, pady=2, sticky='w')
...
analyze_button = ttk.Button(window, text="Analyze", command=analyze_data, style='TButton')
