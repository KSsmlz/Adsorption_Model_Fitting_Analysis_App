...
def analyze_data():
    def create_zoom_command(ax, zoom_factor):
        return lambda: zoom_plot(ax, zoom_factor)

    def create_reset_all_command(ax, all_C, all_S):
        return lambda: reset_all_data_view(ax, all_C, all_S)

    def create_reset_selected_command(ax, C, S):
        return lambda: reset_selected_data_view(ax, C, S)

    def create_toggle_coords_command(ax, C, S):
        return lambda: toggle_coords1(ax, C, S)
    ...
    try:
        # 添加按鈕：返回所有資料點畫面
        def reset_all_data_view(ax, all_C_values, all_S_values):
        ...
        def reset_selected_data_view(ax, C_values, S_values):
        ...
        def zoom_plot(ax, zoom_factor, x_scale=True, y_scale=True):
        ...
        if model_type == "Langmuir":
            ...
            # Calculate initial x and y limits based on data
            max_x1 = max(all_C_values) if all_C_values else 1
            max_y1 = max(all_S_values) if all_S_values else 1
            initial_xlim1 = (0, max_x1 * 1.1)
            initial_ylim1 = (0, max_y1 * 1.1)
            ax1.set_xlim(initial_xlim1)
            ...
            # Calculate initial x and y limits based on data
            max_x2 = max(all_C_values) if all_C_values else 1
            max_y2 = max(np.array(all_C_values) / np.array(all_S_values)) if all_C_values and all_S_values else 1

            initial_xlim2 = (0, max_x2 * 1.1)
            initial_ylim2 = (0, max_y2 * 1.1)

            ax2.set_xlim(initial_xlim2)
            ...
            button_frame1.pack(side=tk.BOTTOM, pady=5)
            ...
            zoom_in_button1 = ttk.Button(button_frame1, text="+", command=create_zoom_command(ax1, 0.8),
                                         style='TButton',
                                         width=3)
            zoom_in_button1.pack(side=tk.LEFT, padx=2)

            zoom_out_button1 = ttk.Button(button_frame1, text="-", command=create_zoom_command(ax1, 1.25),
                                          style='TButton',
                                          width=3)
            zoom_out_button1.pack(side=tk.LEFT, padx=2)

            reset_all_button1 = ttk.Button(button_frame1, text="View All Data",
                                           command=create_reset_all_command(ax1, all_C_values, all_S_values),
                                           style='TButton', width=18)
            reset_all_button1.pack(side=tk.LEFT, padx=2)

            reset_selected_button1 = ttk.Button(button_frame1, text="View Selected Data",
                                                command=create_reset_selected_command(ax1, C_values, S_values),
                                                style='TButton', width=18)
            reset_selected_button1.pack(side=tk.LEFT, padx=2)

            # 新增顯示/隱藏座標按鈕及邏輯
            show_coords1 = False
            annotations1 = []

            def toggle_coords1(ax=ax1, C=C_values, S=S_values):
                ...
                if show_coords1:
                    annotations1 = []
                    if isinstance(C, list) and isinstance(S, list):
                        for c, s in zip(C, S):
                            c_str = f"{c:.4f}" if c < 10 else f"{c:.1f}"
                            s_str = f"{s:.4f}" if s < 10 else f"{s:.1f}"
                            annotation = ax.annotate(f'({c_str}, {s_str})', xy=(c, s), xytext=(3, 3),
            ...
            coords_button1 = ttk.Button(button_frame1, text="Show Coordinates",
                                        command=create_toggle_coords_command(ax1, C_values, S_values),
                                        style='TButton',
                                        width=18)
            coords_button1.pack(side=tk.LEFT, pady=5, padx=2)

            # Frame for buttons
            button_frame2 = ttk.Frame(frame2)
            button_frame2.pack(side=tk.BOTTOM, pady=5)

            zoom_in_button2 = ttk.Button(button_frame2, text="+", command=create_zoom_command(ax2, 0.8),
                                         style='TButton',
                                         width=3)
            zoom_in_button2.pack(side=tk.LEFT, padx=2)

            zoom_out_button2 = ttk.Button(button_frame2, text="-", command=create_zoom_command(ax2, 1.25),
                                          style='TButton',
                                          width=3)
            zoom_out_button2.pack(side=tk.LEFT, padx=2)

            reset_all_button2 = ttk.Button(button_frame2, text="View All Data",
                                           command=create_reset_all_command(ax2, all_C_values,
                                                                            np.array(all_C_values) / np.array(
                                                                                all_S_values) if all_C_values and all_S_values else []),
                                           style='TButton', width=18)
            reset_all_button2.pack(side=tk.LEFT, padx=2)

            reset_selected_button2 = ttk.Button(button_frame2, text="View Selected Data",
                                                command=create_reset_selected_command(ax2, C_values,
                                                                                      np.array(C_values) / np.array(
                                                                                          S_values) if C_values and S_values else []),
                                                style='TButton', width=18)
            reset_selected_button2.pack(side=tk.LEFT, padx=2)

            show_coords2 = False
            annotations2 = []

            def toggle_coords2(ax=ax2, C=C_values,
                               S=np.array(C_values) / np.array(S_values) if C_values and S_values else []):
                ...
                if show_coords2:
                    ...
                    if isinstance(C, list) and isinstance(S, list):
                        for c, cs in zip(C, S):
                            ...
                            annotation = ax.annotate(f'({c_str}, {cs_str})', xy=(c, cs), xytext=(3, 3),
                                                     textcoords='offset points', fontsize=8)
            ...
            coords_button2 = ttk.Button(button_frame2, text="Show Coordinates",
                                        command=create_toggle_coords_command(ax2, C_values,
                                                                             np.array(
                                                                                 C_values) / np.array(
                                                                                 S_values) if C_values and S_values else []),
                                        style='TButton',
                                        width=18)
            coords_button2.pack(side=tk.LEFT, pady=5, padx=2)


        elif model_type == "Freundlich":
            def create_toggle_coords_command(ax, C, S):
                return lambda: toggle_coords3(ax, C, S)
            ...
            # Calculate initial x and y limits based on data
            max_x3 = max(all_C_values) if all_C_values else 1
            max_y3 = max(all_S_values) if all_S_values else 1
            initial_xlim3 = (0, max_x3 * 1.1)
            initial_ylim3 = (0, max_y3 * 1.1)
            ax3.set_xlim(initial_xlim3)
            ax3.set_ylim(initial_ylim3)
            ...
            # Calculate initial x and y limits based on data
            C_log_values_all = np.array(all_C_values)
            S_log_values_all = np.array(all_S_values)
            C_log_values_all = C_log_values_all[C_log_values_all > 0]
            S_log_values_all = S_log_values_all[S_log_values_all > 0]
            max_x4 = max(np.log10(C_log_values_all)) if len(C_log_values_all) > 0 else 1
            max_y4 = max(np.log10(S_log_values_all)) if len(S_log_values_all) > 0 else 1

            initial_xlim4 = (0, max_x4 * 1.1)
            initial_ylim4 = (0, max_y4 * 1.1)
            ax4.set_xlim(initial_xlim4)
            ax4.set_ylim(initial_ylim4)
            ...
            canvas_widget3.pack(fill=tk.BOTH, expand=True, padx=20, pady=5)
            canvas3.draw()
            ...
            button_frame3.pack(side=tk.BOTTOM, pady=5)

            zoom_in_button3 = ttk.Button(button_frame3, text="+", command=create_zoom_command(ax3, 0.8),
                                         style='TButton',
                                         width=3)
            zoom_in_button3.pack(side=tk.LEFT, padx=2)

            zoom_out_button3 = ttk.Button(button_frame3, text="-", command=create_zoom_command(ax3, 1.25),
                                          style='TButton',
                                          width=3)
            zoom_out_button3.pack(side=tk.LEFT, padx=2)

            reset_all_button3 = ttk.Button(button_frame3, text="View All Data",
                                           command=create_reset_all_command(ax3, all_C_values, all_S_values),
                                           style='TButton', width=18)
            reset_all_button3.pack(side=tk.LEFT, padx=2)

            reset_selected_button3 = ttk.Button(button_frame3, text="View Selected Data",
                                                command=create_reset_selected_command(ax3, C_values, S_values),
                                                style='TButton', width=18)
            reset_selected_button3.pack(side=tk.LEFT, padx=2)
            ...
            def toggle_coords3(ax=ax3, C=C_values, S=S_values):
            ...
            coords_button3 = ttk.Button(button_frame3, text="Show Coordinates",
                                        command=create_toggle_coords_command(ax3, C_values, S_values),
                                        style='TButton',
                                        width=18)
            coords_button3.pack(side=tk.LEFT, pady=5, padx=2)

            # Frame for buttons
            button_frame4 = ttk.Frame(frame4)
            button_frame4.pack(side=tk.BOTTOM, pady=5)

            # For log-scale, we need to filter out 0 or negative number to avoid error
            C_log_values_all = np.array(all_C_values)
            S_log_values_all = np.array(all_S_values)
            C_log_values_all = C_log_values_all[C_log_values_all > 0]
            S_log_values_all = S_log_values_all[S_log_values_all > 0]
            ...
            zoom_in_button4 = ttk.Button(button_frame4, text="+",
                                         command=create_zoom_command(ax4, 0.8),
                                         style='TButton', width=3)
            zoom_in_button4.pack(side=tk.LEFT, padx=2)

            zoom_out_button4 = ttk.Button(button_frame4, text="-",
                                          command=create_zoom_command(ax4, 1.25),
                                          style='TButton', width=3)
            zoom_out_button4.pack(side=tk.LEFT, padx=2)

            reset_all_button4 = ttk.Button(button_frame4, text="View All Data",
                                           command=create_reset_all_command(ax4, np.log10(
                                               C_log_values_all) if len(
                                               C_log_values_all) > 0 else [
                                               0], np.log10(
                                               S_log_values_all) if len(
                                               S_log_values_all) > 0 else [
                                               0]),
                                           style='TButton', width=18)
            reset_all_button4.pack(side=tk.LEFT, padx=2)

            reset_selected_button4 = ttk.Button(button_frame4, text="View Selected Data",
                                                command=create_reset_selected_command(ax4, np.log10(
                                                    C_log_values_selected) if len(C_log_values_selected) > 0 else [0],
                                                                                      np.log10(
                                                                                          S_log_values_selected) if len(
                                                                                          S_log_values_selected) > 0 else [
                                                                                          0]),
                                                style='TButton',
                                                width=18)
            reset_selected_button4.pack(side=tk.LEFT, padx=2)


            show_coords4 = False
            annotations4 = []

            def toggle_coords4(ax=ax4, C=C_log_values_selected, S=S_log_values_selected):
                ...
                if show_coords4:
                    ...
                    if isinstance(C, list) and isinstance(S, list):
                        for c, s in zip(C, S):
                            c_str = f"{c:.4f}" if c < 10 else f"{c:.1f}"
                            s_str = f"{s:.4f}" if s < 10 else f"{s:.1f}"
                            annotation = ax.annotate(f'({c_str}, {s_str})', xy=(np.log10(c), np.log10(s)),
                                                     xytext=(3, 3),
                                                     textcoords='offset points', fontsize=8)
            ...
            coords_button4 = ttk.Button(button_frame4, text="Show Coordinates",
                                        command=create_toggle_coords_command(ax4, C_log_values_selected,
                                                                             S_log_values_selected),
                                        style='TButton',
                                        width=18)
            coords_button4.pack(side=tk.LEFT, pady=5, padx=2)
