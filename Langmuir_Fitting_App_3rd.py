...
def analyze_langmuir(C, S):
    ...
    # Plotting
    fig, ax = plt.subplots()
    ax.scatter(C, S, label='All Data Points')
    ax.plot(C_smooth, S_fit, 'r-', label='Langmuir Fit')
...
def delete_data():
    """Deletes selected data from the data frame."""
...
def analyze_data():
    """Analyzes data from the data frame and displays the results."""
    ...
    try:
        ...
        # Plot selected data
        fig.axes[0].scatter([float(c) for c in C_values], [float(s) for s in S_values], color='green', label='Selected Data Points')
        fig.axes[0].legend()
        canvas.draw()
